...
def colormap_idd20k(n):
    cmap=np.zeros([n, 3]).astype(np.uint8)
    cmap[0,:] = np.array([128, 64,128])#road
    cmap[1,:] = np.array([ 81,  0, 81])#drivable fallback
    cmap[2,:] = np.array([244, 35,232])#sidewalk
    cmap[3,:] = np.array([152,251,152])#non-drivable fallback
    cmap[4,:] = np.array([220, 20, 60])#pedestrian
    
    cmap[5,:] = np.array([255,  0,  0])#rider
    cmap[6,:] = np.array([  0,  0,230])#motorcycle
    cmap[7,:] = np.array([119, 11, 32])#bicyle
    cmap[8,:] = np.array([255,204, 54])#autorichshaw
    cmap[9,:] = np.array([  0,  0,142])#car

    cmap[10,:]= np.array([  0,  0, 70])#truck
    cmap[11,:]= np.array([  0, 60,100])#bus
    cmap[12,:]= np.array([136,143,153])#vehicle fallback
    cmap[13,:]= np.array([220,190, 40])#curb
    cmap[14,:]= np.array([102,102,156])#wall
    
    cmap[15,:]= np.array([190,153,153])#fence
    cmap[16,:]= np.array([180,165,180])#guard rail
    cmap[17,:]= np.array([174, 64, 67])#billboard
    cmap[18,:]= np.array([220,220,  0])#traffic sign
    cmap[19,:]= np.array([250,170, 30])#traffic light
    
    cmap[20,:]= np.array([153,153,153])#pole
    cmap[21,:]= np.array([169,187,214])#obs
    cmap[22,:]= np.array([ 70, 70, 70])#building
    cmap[23,:]= np.array([150,120, 90])#bridge
    cmap[24,:]= np.array([107,142, 35])#vegetation

    cmap[25,:]= np.array([ 70,130,180])#sky
    cmap[26,:]= np.array([  0,  0,  0])
