...
def train(args, get_dataset, model, enc=False):
	...
	dataset_val = {dname: get_dataset(dname, 'val', 2000) for dname in datasets}
	# dataset_unlabeled = {dname: get_dataset(dname, co_transform, 'train_extra' , mode='unlabeled') for dname in datasets}
	dataset_unlabeled = {dname: get_dataset(dname, 'test'  , mode='unlabeled') for dname in datasets}
	...
	if args.model == 'erfnet_pspnet':
		optimizer = Adam(model.parameters(), 1e-4, (0.9, 0.999),  eps=1e-08, weight_decay=2e-4)      ## scheduler 2
		...
		if (epoch == 1) or (epoch%1==0): ## validation after every 5 epoch
			for phase in ['val']:
				...
				if (eval_iou):
					...
					print ("curb    : %.6f" % (iou_classes[13]*100.0), "%\t")
					print ("fence   : %.6f" % (iou_classes[14]*100.0), "%\t")
					print ("wall    : %.6f" % (iou_classes[15]*100.0), "%\t")
					print ("building: %.6f" % (iou_classes[16]*100.0), "%\t")
					print ("person  : %.6f" % (iou_classes[17]*100.0), "%\t")
					print ("motorcyl: %.6f" % (iou_classes[18]*100.0), "%\t")
					print ("bicyclis: %.6f" % (iou_classes[19]*100.0), "%\t")
					print ("sky     : %.6f" % (iou_classes[20]*100.0), "%\t")
					print ("vege    : %.6f" % (iou_classes[21]*100.0), "%\t")
					print ("terrain : %.6f" % (iou_classes[22]*100.0), "%\t")
					print ("markings: %.6f" % (iou_classes[23]*100.0), "%\t")
					print ("crosszeb: %.6f" % (iou_classes[24]*100.0), "%\t")

					iou_classes=iou['IDD20K'][1]
					print ("road    : %.6f" % (iou_classes[0]*100.0), "%\t")
					print ("drivable: %.6f" % (iou_classes[1]*100.0), "%\t")
					print ("sidewalk: %.6f" % (iou_classes[2]*100.0), "%\t")
					print ("nondriv : %.6f" % (iou_classes[3]*100.0), "%\t")
					print ("pedestr : %.6f" % (iou_classes[4]*100.0), "%\t")
					print ("rider   : %.6f" % (iou_classes[5]*100.0), "%\t")
					print ("motor   : %.6f" % (iou_classes[6]*100.0), "%\t")
					print ("bicycle : %.6f" % (iou_classes[7]*100.0), "%\t")
					print ("rickshaw: %.6f" % (iou_classes[8]*100.0), "%\t")
					print ("car     : %.6f" % (iou_classes[9]*100.0), "%\t")
					print ("truck   : %.6f" % (iou_classes[10]*100.0), "%\t")
					print ("bus     : %.6f" % (iou_classes[11]*100.0), "%\t")
					print ("vehicle : %.6f" % (iou_classes[12]*100.0), "%\t")
					print ("curb    : %.6f" % (iou_classes[13]*100.0), "%\t")
					print ("wall    : %.6f" % (iou_classes[14]*100.0), "%\t")
					print ("fence   : %.6f" % (iou_classes[15]*100.0), "%\t")
					print ("guard   : %.6f" % (iou_classes[16]*100.0), "%\t")
					print ("bboard  : %.6f" % (iou_classes[17]*100.0), "%\t")
					print ("tsign   : %.6f" % (iou_classes[18]*100.0), "%\t")
					print ("tlight  : %.6f" % (iou_classes[19]*100.0), "%\t")
					print ("pole    : %.6f" % (iou_classes[20]*100.0), "%\t")
					print ("obs     : %.6f" % (iou_classes[21]*100.0), "%\t")
					print ("building: %.6f" % (iou_classes[22]*100.0), "%\t")
					print ("bridge  : %.6f" % (iou_classes[23]*100.0), "%\t")
					print ("vege    : %.6f" % (iou_classes[24]*100.0), "%\t")
					print ("sky     : %.6f" % (iou_classes[25]*100.0), "%\t")
		############# VALIDATION ends #######################
